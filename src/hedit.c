...
HEDIT(hedit_level)
{
	...
	{
		printf_to_char( ch, "HEdit : level must from range -1 to %d.\n\r", MAX_LEVEL );
		return FALSE;
	}
...
HEDIT(hedit_new)
{
	...
	{
		send_to_char( "HEdit : help ju¿ istnieje.\n\r", ch );
		return FALSE;
	}
...
void hedit( CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("HEdit: Masz za niskie security.\n\r",ch);
	edit_done(ch);
	return;
    }
	...
	if (!str_prefix(command, hedit_table[cmd].name) )
	{
		if ((*hedit_table[cmd].olc_fun) (ch, argument))
		{
		 ...
		 if(had->area)
		     SET_BIT(had->area->area_flags, AREA_CHANGED);
...
void do_hedit(CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char( "HEdit : Help nie istnieje.\n\r", ch );
		return;
	}
